...
import { LEADS, LEAD_ANGLES, MAX_DEFLECTION } from './axisConstants';
...
/**
 * Pattern types for QRS deflections
 */
enum QRSPatternType {
  R_DOMINANT = "r_dominant",
  QS_DOMINANT = "qs_dominant",
...
/**
 * Validates all deflections to ensure they maintain reasonable relationships
 * while satisfying both Einthoven's law and area-under-curve principles
 */
function validateDeflections(deflections: LimbLeadDeflections): LimbLeadDeflections {
  // First apply Einthoven's law validation
  const validatedDeflections = validateEinthovenLaw(deflections);
  
  // Get projections for each lead to identify strong negative projections
  const qrsAxisAngle = estimateAxisAngleFromDeflections(validatedDeflections);
  const projections: Record<string, number> = {};
  LEADS.forEach(lead => {
    projections[lead] = calculateLeadProjection(qrsAxisAngle, LEAD_ANGLES[lead as keyof typeof LEAD_ANGLES]);
  });
  
  // Then check and fix area-under-curve relationships for each lead
  LEADS.forEach(lead => {
    const leadKey = lead as keyof LimbLeadDeflections;
    const deflection = validatedDeflections[leadKey];
    const projection = projections[lead];
    
    // Check if deflection exists
    if (!deflection) {
      // Create default values for missing deflection
      validatedDeflections[leadKey] = { 
        q: -MIN_CLINICAL_VALUES.Q_AMPLITUDE, 
        r: MIN_CLINICAL_VALUES.R_AMPLITUDE, 
        s: -MIN_CLINICAL_VALUES.S_AMPLITUDE 
      };
      return; // Skip to next lead
    }
    
    // Calculate area under curve (simplified model)
    const rArea = deflection.r;
    const qsArea = Math.abs(deflection.q) + Math.abs(deflection.s);
    
    // For strong negative projections (axis opposing lead direction), ensure Q+S dominance
    if (projection < -0.5) {
      // Desired ratio based on how negative the projection is
      // Increase the ratio even more for very negative projections
      const desiredRatio = 2.0 + Math.min(3.0, Math.abs(projection) * 2.5); // Increased from 1.5/1.5/1.5 to 2.0/3.0/2.5
      
      // Check if QS is sufficiently dominant
      if (qsArea / rArea < desiredRatio) {
        // Make QS more dominant by boosting QS and/or reducing R
        const targetQSArea = rArea * desiredRatio;
        const boostFactor = targetQSArea / qsArea;
        
        // Calculate R reduction factor based on how negative the projection is
        const rReductionFactor = Math.max(0.5, 0.8 - Math.abs(projection) * 0.5); // More reduction for more negative projections
        
        // Boost Q and S waves to achieve desired dominance
        validatedDeflections[leadKey] = {
          q: deflection.q * boostFactor,
          r: deflection.r * rReductionFactor, // More aggressive R reduction
          s: deflection.s * boostFactor
        };
      }
      
      // For extremely negative projections (< -0.8), ensure R is minimal
      if (projection < -0.8) {
        const extremeDeflection = validatedDeflections[leadKey];
        const qsAmplitude = Math.abs(extremeDeflection.q) + Math.abs(extremeDeflection.s);
        
        // Ensure R is no more than 15% of Q+S for extremely negative projections
        if (extremeDeflection.r > qsAmplitude * 0.15) {
          validatedDeflections[leadKey] = {
            q: extremeDeflection.q,
            r: qsAmplitude * 0.15,
            s: extremeDeflection.s
          };
        }
      }
      
      // Ensure the final total deflection has sufficient amplitude
      const updatedDeflection = validatedDeflections[leadKey];
      const totalAmplitude = Math.abs(updatedDeflection.q) + updatedDeflection.r + Math.abs(updatedDeflection.s);
      
      if (totalAmplitude < MIN_CLINICAL_VALUES.TOTAL_AMPLITUDE) {
        const scaleFactor = MIN_CLINICAL_VALUES.TOTAL_AMPLITUDE / totalAmplitude;
        validatedDeflections[leadKey] = {
          q: updatedDeflection.q * scaleFactor,
          r: updatedDeflection.r * scaleFactor,
          s: updatedDeflection.s * scaleFactor
        };
      }
    }
    // For perpendicular projections (isoelectric), balance areas
    else if (Math.abs(projection) < 0.15) {
      // Ensure R area ≈ Q+S area for isoelectric appearance
      const targetRArea = qsArea * 1.1; // Slight dominance for R
    ...
    // For strong positive projections (axis aligned with lead), ensure R dominance
    else if (projection > 0.5) {
      // Desired ratio based on how positive the projection is
      const desiredRatio = 1.5 + Math.min(1.5, projection * 1.5);
      
      // Check if R is sufficiently dominant
      if (rArea / qsArea < desiredRatio) {
        // Make R more dominant
        const targetRArea = qsArea * desiredRatio;
        
        validatedDeflections[leadKey] = {
          q: deflection.q * 0.9, // Slightly reduce Q
          r: targetRArea,
          s: deflection.s * 0.9  // Slightly reduce S
        };
      }
    ...
    // For intermediate cases, ensure minimum total amplitude
    else {
      if (rArea + qsArea < MIN_CLINICAL_VALUES.TOTAL_AMPLITUDE) {
        const scaleFactor = MIN_CLINICAL_VALUES.TOTAL_AMPLITUDE / (rArea + qsArea);
        validatedDeflections[leadKey] = {
          q: deflection.q * scaleFactor,
          r: deflection.r * scaleFactor,
          s: deflection.s * scaleFactor
        };
      }
    }
  });
...
/**
 * Estimate the QRS axis angle from the deflections
 * Used to determine projections for validation
 */
function estimateAxisAngleFromDeflections(deflections: LimbLeadDeflections): number {
  // Verify that Lead III follows Einthoven's law
  // If it doesn't, adjust our approach to calculation
  const leadIIIFromEinthoven = {
    q: 0, // Always zero for educational purposes
    r: deflections.leadII.r - deflections.leadI.r,
    s: deflections.leadII.s - deflections.leadI.s
  };
  
  // If Lead III doesn't match Einthoven's law, use the calculated value instead
  // Only check R and S wave values since Q is always zero
  const useEinthovenLeadIII = Math.abs(deflections.leadIII.r - leadIIIFromEinthoven.r) > 0.1 ||
                             Math.abs(deflections.leadIII.s - leadIIIFromEinthoven.s) > 0.1;
  
  const leadIIIForCalculation = useEinthovenLeadIII ? leadIIIFromEinthoven : deflections.leadIII;

  // Calculate net areas for all six limb leads
  const calculateNetArea = (deflection: QRSDeflection): number => {
    // Only consider R and S waves for educational purposes
    return deflection.r - Math.abs(deflection.s);
  };

  const leadINet = calculateNetArea(deflections.leadI);
  const leadIINet = calculateNetArea(deflections.leadII);
  const leadIIINet = calculateNetArea(leadIIIForCalculation);
  const aVRNet = calculateNetArea(deflections.aVR);
  const aVLNet = calculateNetArea(deflections.aVL);
  const aVFNet = calculateNetArea(deflections.aVF);

  // Calculate axis using leads I and II as the primary calculation
  // These are at 0° and +60° respectively, providing a good basis for calculation
  const x = leadINet;                // Horizontal component (Lead I at 0°)
  const y = leadIINet - (leadINet/2); // Vertical component (Lead II minus half of Lead I to get the pure vertical component)
  ...
  // Validate the calculation using Einthoven's triangle equations
  // If the calculated axis doesn't match the input deflections well,
  // we can refine it by checking projections against multiple leads
  const leadAngles = {
    leadI: 0,
    leadII: 60,
    leadIII: 120,
  ...
  // Calculate mean squared error of our angle estimate
  let mse = 0;
  let count = 0;
  
  Object.entries(leadAngles).forEach(([lead, angle]) => {
    const leadKey = lead as keyof LimbLeadDeflections;
    const expectedProjection = calculateLeadProjection(angleDegrees, angle);
    const actualNet = calculateNetArea(deflections[leadKey]);
    
    // Normalize both values for comparison
    const maxNet = Math.max(...[leadINet, leadIINet, leadIIINet, Math.abs(aVRNet), aVLNet, aVFNet].map(Math.abs));
    const normalizedExpected = expectedProjection;
    const normalizedActual = actualNet / maxNet;
    
    mse += Math.pow(normalizedExpected - normalizedActual, 2);
    count++;
  });
  
  mse = mse / count;
  
  // If MSE is too high, try to refine our estimate
  if (mse > 0.15) {
    // Use a weighted average approach with all leads
    let weightedSumX = 0;
    let weightedSumY = 0;
    let sumWeights = 0;
    ...
    Object.entries(leadAngles).forEach(([lead, angle]) => {
      const leadKey = lead as keyof LimbLeadDeflections;
      const netArea = calculateNetArea(deflections[leadKey]);
      const weight = Math.abs(netArea);
      
      // Convert lead angle to radians
      const angleRad = (angle * Math.PI) / 180;
      
      weightedSumX += Math.cos(angleRad) * netArea;
      weightedSumY += Math.sin(angleRad) * netArea;
      sumWeights += weight;
    });
    
    if (sumWeights > 0) {
      const resultAngleRadians = Math.atan2(weightedSumY, weightedSumX);
      angleDegrees = (resultAngleRadians * 180) / Math.PI;
